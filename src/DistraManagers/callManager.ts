...
export default class RTCManager {
  ...
  constructor({ room }: { room: Room; roomId: string }) {
    ...
    room.onPeerStream((stream, peerId) => {
      ...
      stream.addEventListener("removetrack", () => {
        // videoParent.remove();
        useCallPrefsState.getState().removeBubbleWithId(peerId);
    ...
    getRoomJoined((type, id) => {
      if (type === "view") {
        useCallPrefsState.getState().addVideoBubble({
          id,
          stream: new MediaStream(),
          isAudioOnly: false,
        });
      } else if (type === "cutStream") {
        useCallPrefsState.getState().removeBubbleWithId(id);
      }
  ...
  public shareMedia = async (type: RoomActionType) => {
    if (type === "cutStream") {
      ...
      useCallPrefsState.getState().clearVideoBubbles();

      this.joinRoom("cutStream");

      useCallPrefsState.getState().setMyStream(null);

      useCallPrefsState.getState().setCallConsent(false);
    } else if (type === "view") {
      useCallPrefsState.getState().setCallConsent(true);
      this.joinRoom("view");
      useCallPrefsState.getState().setMyStream(null);
    } else {
      useCallPrefsState.getState().setCallConsent(true);
      ...
      if (selfStream) {
        ...
        this.joinRoom(type);
        useCallPrefsState.getState().setMyStream(selfStream);
      }
    }
  };
}
