...
import { ActionsType, ExtendedInstance } from "../../../helpers/types/distraTypes";
...
export const useRoomStateManager = create<RoomStateManager>((set) => ({
	peerMap: {},
	addToPeerMap: (id: string, peer: ExtendedInstance) => set((state) => {
		state.peerMap[id] = peer;
		return state;
	}),
	removeFromPeerMap: (id: string) => set((state) => {
		delete state.peerMap[id];
		return state;
	}),

	actions: {},
	setActions: (type:string, actions: ActionsType) => set((state) => {
		state.actions[type] = actions;
		return state;
	}),

	pendingTransmissions: {},
	addToPendingTransmissions: (id: string, targetId: string, transmission: any) => set((state) => {
	...
	removeFromPendingTransmissions: (id: string) => set((state) => {
