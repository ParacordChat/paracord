...
interface HookStateManager {
  ...
  setOnPeerTrack: (arg0: (track: any, stream: any, id: string, meta: any) => void) => void;
...
export const useHookStateManager = create<HookStateManager>((set) => ({
	onPeerJoin: noOp,
	setOnPeerJoin: (onPeerJoin: (id: string) => void) => set((state) => {
		state.onPeerJoin = onPeerJoin;
		return state;
	}),
	onPeerLeave: noOp,
	setOnPeerLeave: (onPeerLeave: (id: string) => void) => set((state) => {
		state.onPeerLeave = onPeerLeave;
		return state;
	}),
	onPeerStream: noOp,
	setOnPeerStream: (onPeerStream: (stream: any, id: string, meta: any) => void) => set((state) => {
		state.onPeerStream = onPeerStream;
		return state;
	}),
	onPeerTrack: noOp,
	setOnPeerTrack: (onPeerTrack: (track: any, stream: any, id: string, meta: any) => void) => set((state) => {
		state.onPeerTrack = onPeerTrack;
		return state;
	}),
	onPeerError: noOp,
	setOnPeerError: (onPeerError: (id: string, error: any) => void) => set((state) => {
