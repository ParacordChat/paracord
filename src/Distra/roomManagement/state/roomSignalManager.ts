...
export const useRoomSignalManager = create<RoomSignalManager>((set) => ({
	pendingPongs: {},
	addToPendingPongs: (id: string, pong: (value?: unknown) => void) =>
		set((state) => {
			state.pendingPongs[id] = pong;
			return state;
		}),
	removeFromPendingPongs: (id: string) =>
		set((state) => {
	...
	pendingStreamMetas: {},
	addToPendingStreamMetas: (id: string, meta: any) =>
		set((state) => {
			state.pendingStreamMetas[id] = meta;
			return state;
		}),
	removeFromPendingStreamMetas: (id: string) =>
		set((state) => {
	...
	pendingTrackMetas: {},
	addToPendingTrackMetas: (id: string, meta: any) =>
		set((state) => {
			state.pendingTrackMetas[id] = meta;
			return state;
		}),
	removeFromPendingTrackMetas: (id: string) =>
		set((state) => {
